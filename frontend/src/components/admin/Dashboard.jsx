...
const AdminDashboard = () => {
  ...
  const StatCard = ({ icon: Icon, label, value, trend, bgGradient, iconBg }) => (
    <motion.div 
      ...
      className="glass-card-strong rounded-2xl p-6 shadow-lg hover:shadow-2xl transition-all duration-300"
    >
      <div className="flex items-start justify-between">
        <div>
          ...
          {trend !== undefined && (
            <div className={`flex items-center gap-1 mt-2 text-sm font-medium ${trend >= 0 ? 'text-emerald-300' : 'text-red-300'}`}>
  ...
  return (
    ...
    >
      ...
      >
        ...
        <motion.div variants={chartVariants} className="glass-card rounded-2xl shadow-lg hover:shadow-2xl transition-all p-6">
        ...
        {/* Status Distribution */}
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.15 }} className="glass-card rounded-2xl shadow-lg hover:shadow-2xl transition-all p-6">
        ...
        <motion.div variants={containerVariants} initial="hidden" animate="visible" className="space-y-4">
          <motion.div variants={itemVariants} className="glass-card-light rounded-2xl p-6 shadow-lg hover:shadow-2xl transition-all">
            <p className="text-emerald-300 text-sm font-semibold">Resolution Rate</p>
            <p className="text-4xl font-bold text-white mt-2">{resolutionRate}%</p>
            <div className="flex items-center gap-1 mt-2 text-sm text-emerald-300 font-medium">
              <ArrowUpRight className="w-4 h-4" />
              {resolutionTrend}% improvement
            </div>
          </motion.div>
          <motion.div variants={itemVariants} className="glass-card-light rounded-2xl p-6 shadow-lg hover:shadow-2xl transition-all">
            <p className="text-blue-300 text-sm font-semibold">Avg Resolution Time</p>
            <p className="text-4xl font-bold text-white mt-2">{avgResolutionTime}</p>
            <p className="text-xs text-blue-200 mt-2">Days on average</p>
          </motion.div>
          <motion.div variants={itemVariants} className="glass-card-light rounded-2xl p-6 shadow-lg hover:shadow-2xl transition-all">
            <p className="text-purple-300 text-sm font-semibold">Active Engineers</p>
            <p className="text-4xl font-bold text-white mt-2">{users.filter(u => u.role === 'engineer').length}</p>
            <p className="text-xs text-purple-200 mt-2">Currently assigned</p>
          </motion.div>
        </motion.div>
      </motion.div>
      ...
      <motion.div 
        ...
        className="glass-card rounded-2xl shadow-lg hover:shadow-2xl transition-all p-6"
      >
        <motion.h3 
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
        ...
        <div className="space-y-3">
          {recentIssues.map((issue, index) => (
            <motion.div
              ...
              whileHover={{ x: 8, boxShadow: '0 8px 20px rgba(12,30,63,0.4)' }}
              className="flex items-center justify-between p-4 rounded-xl glass-card-light hover:glass-card transition-all cursor-pointer"
            >
