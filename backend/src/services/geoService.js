...
/**
 * Get ward from coordinates using PostGIS
...
async function getWardFromCoordinates(latitude, longitude) {
    try {
        ...
        const result = await db.query(
            'SELECT * FROM get_ward_from_coordinates($1, $2)',
            [latitude, longitude]
        );
        ...
        logger.info(`Ward ${ward.ward_number} found for coordinates: ${latitude}, ${longitude}`);
...
async function checkDuplicateIssues(latitude, longitude, issueTypeId, submittedAt = new Date()) {
    try {
        ...
        // Call PostGIS duplicate detection function
        const result = await db.query(
            'SELECT * FROM check_duplicate_issue($1, $2, $3, $4)',
            [latitude, longitude, issueTypeId, submittedAt]
        );
        
        if (result.rows.length > 0) {
            logger.warn(`Found ${result.rows.length} duplicate issues nearby`);
        }
